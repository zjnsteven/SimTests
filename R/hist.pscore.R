hist.pscore <- function(x, numdraws=5000, xlab="Propensity Score", main=NULL,
                        freq=F, xlim = NULL,...) {
  ...
  if (is.null(main)) {
    hist(pscore[treat==1],
         xlim=xlim, xlab=xlab, freq=freq,
         main="Raw Treated", ...)
    hist(pscore.treated.matched,
         xlim=xlim, xlab=xlab, freq=freq,
