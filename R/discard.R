discard <- function(treat, pscore, option, X) {
  ...
  if (is.logical(option)) {
    ...
    # discard units outside of common support
    discarded <- (pscore < max(pmin0, pmin1) | pscore > min(pmax0, pmax1))
  } else if (option == "control") {
    # discard control units only
    discarded <- (pscore < pmin1 | pscore > pmax1)
  } else if (option == "treat") {
    # discard treated units only
    ...
    if (option == "hull.control") {
      # discard units not in T convex hull
    ...
    } else if (option == "hull.both") {
      # discard units not in T&C convex hull
